...
contains
  ...
  subroutine epsinv(nn,ndim,nang,gam,stc,c,occ,dacc, &
    ...
    real*8     gam(nAng,*),stc(nDim,*),c(nDim,*),occ(*),dacc 
    real*8     qij(*),wij(nDim,*),epsi(nAng,*)
    complex*16 omega
    integer  izp(nn),lmax(nType)
    integer, intent(in) :: nType

    !     Local
    integer    n,m,info,ipiv(nAng)
    real*8     wrk(64*nAng),plr(nAng,nAng),epsmac,tmp
    ...
    call polar(nn,ndim,nang,stc,c,occ,dacc,qij,wij,omega,plr,brd,lmax,izp,nType)
    do m = 1,nang
      do n = 1,m-1
        epsi(m,n) = 0.d0
        epsi(n,m) = 0.d0
