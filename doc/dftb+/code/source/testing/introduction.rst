...
- **Regression Testing Benefits**
  ...
  - Safeguards existing functionality when integrating new features.
...
run tests in software projects. Here’s how they can be used for a
testing setup that includes regression tests by default and
conditionally enables unit testing.

Default Behaviour
~~~~~~~~~~~~~~~~~

- Regression tests are included in the build process by default.
  
- These tests ensure that existing functionality remains stable during
  development.
...
When this variable is set, the CMake configuration includes the unit
test targets. Then compile the code and run tests in the build
directory using:
...
- **Control:** Allows for streamlined testing during different phases
  of development (for example, we differentiate quick and slow
  regression tests).
