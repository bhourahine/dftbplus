...
module dftbp_extlibs_elsirciiface
...
#:endif
  ...
  !> RCI solver choices
  public :: RCI_SOLVER, RCI_SOLVER_DAVIDSON, RCI_SOLVER_OMM, RCI_SOLVER_PPCG, RCI_SOLVER_CHEBFILTER
  !> Constant for ijob
  public :: RCI_INIT_IJOB
  !> Task labels for RCI solver operations
...
#:if not WITH_ELSI_RCI
  ...
  !> Generates error message, if a routine stub was called
  ...
  !> Dummy initialisation for RCI structures
  ...
  !> Dummy memory allocation for RCI structures
  ...
  !> Dummy memory deallocation for RCI structures
...
end module dftbp_extlibs_elsirciiface
