...
#:endif
  ...
  interface unpackHS
    ...
    module procedure unpackHS_cmplx_kpts_coord
...
contains
  ...
  !> Unpacks sparse matrix to square form (complex version) Note the non on-site blocks are only
  !! filled in the lower triangle part of the matrix. Uses coordinate-including gauge convention
  subroutine unpackHS_cmplx_kpts_coord(square, orig, kPoint, coords, iNeighbour, nNeighbourSK,&
      & iAtomStart, iSparseStart, img2CentCell)
    ...
    !> Coordinates of all of the atoms, including images
    ...
    do iAtom1 = 1, nAtom
      ...
      do iNeigh = 0, nNeighbourSK(iAtom1)
        ...
        @:ASSERT(jj >= ii)
        nOrb2 = iAtomStart(iAtom2f + 1) - jj
        phase = exp(imag * dot_product(kPoint2p, coords(:, iAtom2) - coords(:, iAtom1)))
        square(jj:jj+nOrb2-1, ii:ii+nOrb1-1) = square(jj:jj+nOrb2-1, ii:ii+nOrb1-1)&
            & + phase * reshape(orig(iOrig:iOrig+nOrb1*nOrb2-1), [nOrb2, nOrb1])
      end do
    end do

  end subroutine unpackHS_cmplx_kpts_coord
  ...
  subroutine unpackHS_cmplx_kpts_peierls(square, orig, kPoint, tdVecPot, coordAll, iNeighbour, &
    ...
    !> Coordinates of the atoms (3, nAllAtom)
