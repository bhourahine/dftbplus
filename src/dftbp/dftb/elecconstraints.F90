...
module dftbp_dftb_elecconstraints
  use dftbp_common_accuracy, only : dp, hugeIterations, rsp
...
!  end subroutine addConstraintsShiftS_
  ...
  subroutine addConstraintsShiftL_(iShift,qBlockSkew, orb, species, conAt, conSh, Ltarget, V, vec)
   ...
   real(dp), intent(in) :: vec(3)
    ...
    allocate(Lz(orb%mOrb,orb%mOrb), source=(0.0_dp,0.0_dp))
    allocate(Lplus(orb%mOrb,orb%mOrb), source=(0.0_dp,0.0_dp))
  ...
  subroutine constrainMj_(shift, qIn, iShift, qBlockSkew, orb, species, conAt, conSh, Jtarget, V,&
      & vec)
    real(dp), intent(inout)     :: shift(:,:,:,:)
    real(dp), intent(in)        :: qIn(:,:,:)
    real(dp), intent(inout)     :: iShift(:,:,:,:)
