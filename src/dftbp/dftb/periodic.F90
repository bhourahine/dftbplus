...
#:endif
  use dftbp_geometry_boundarycond, only : boundaryCondsEnum, TBoundaryConds, zAxis
...
#:if WITH_UNIT_TESTS
  ...
  ! from the outside.
  public :: allocateNeighbourArrays, distributeAtoms, reallocateNeighbourSizedArrays
  public :: fillNeighbourArrays
#:endif
...
contains
  ...
  subroutine getLatticePoints(latPoint, latVec, recVec2p, cutoff, boundaryConds, posExtension,&
    ...
    !> Return only those lattice points which are really not outside the given distance cutoff.
    ...
    if (present(boundaryConds)) then
      call TLatPointIter_init(latPointGen, latVec, recVec2p, cutoff, negExtension, posExtension,&
  ...
  subroutine updateNeighbourListAndSpecies(env, coord, species, img2CentCell, iCellVec, neigh,&
      & nAllAtom, coord0, species0, cutoff, rCellVec, boundaryConds, errStatus, symmetric,&
      & helicalBoundConds)
    ...
    !> Cutoff distance for which neighbourlist should be created
    real(dp), intent(in) :: cutoff
    ...
    call updateNeighbourList(coord, img2CentCell, iCellVec, neigh, nAllAtom, coord0, cutoff,&
        & rCellVec, boundaryConds, errStatus, env, symmetric, helicalBoundConds)
  ...
  #:endif
    ...
    if (boundaryConds%iBoundaryCondition == boundaryCondsEnum%cluster) then
      call update(coord0, cutoff, errStatus)
      @:PROPAGATE_ERROR(errStatus)
    end if
        ...
        lpIAtom2: do iAtom2 = 1, iAtom2End
          ...
          if (iAtom1 /= oldIAtom1) then
            ...
            if (nAllAtom > mAtom) then
              ...
              call reallocateAtomSizedArrays(img2CentCell, iCellVec, coord, mAtom)
            end if
            coord(:, nAllAtom) = rr
          ...
          if (startAtom <= iAtom2 .and. iAtom2 <= endAtom) then
            ...
            if (neigh%nNeighbour(iAtom2) > maxNeighbour) then
              ...
              call reallocateNeighbourSizedArrays(iNeighbour, neighDist2, maxNeighbour)
            end if
  ...
  #:endif
    ...
    call reallocateAtomSizedArrays(img2CentCell, iCellVec, coord, nAllAtom)
...
99010 format ('Cutoff (', E16.6, ') greater than last cutoff ', '(', E13.6,&
  ...
  subroutine reallocateAtomSizedArrays(img2CentCell, iCellVec, coord, mNewAtom)
  ...
  end subroutine reallocateAtomSizedArrays


  !> Reallocate array which depends on the maximal nr. of neighbours.
  subroutine reallocateNeighbourSizedArrays(iNeighbour, neighDist2, mNewNeighbour)
  ...
  end subroutine reallocateNeighbourSizedArrays
