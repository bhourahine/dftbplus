...
contains
  ...
  subroutine index_mergesort(indx, arr, tolerance)
    ...
    if (n > 1) then
      allocate(tmp(n))
      call merge_sort_index_track(arr, indx, tmp, 1, n, tolerance)
      deallocate(tmp)
    end if
  ...
  !> Recursive engine splitting the index array
  recursive subroutine merge_sort_index_track(arr, indx, tmp, left, right, tol)
    ...
    if (left < right) then
      ...
      call merge_sort_index_track(arr, indx, tmp, left, mid, tol)
      call merge_sort_index_track(arr, indx, tmp, mid + 1, right, tol)

      ! Merge the two sorted index segments
      call merge_index(arr, indx, tmp, left, mid, right, tol)
  ...
  subroutine merge_index(arr, indx, tmp, left, mid, right, tol)
    ...
    ! Cache the current segment into temporary workspace
    do ii = left, right
      tmp(ii) = indx(ii)
    end do
    ...
    do while (ii <= mid .and. jj <= right)

      val_i = arr(tmp(ii))
      val_j = arr(tmp(jj))

      ! 1. If element ii is significantly smaller than element jj
      if ((val_j - val_i) > tol) then
        indx(kk) = tmp(ii)
        ii = ii + 1

        ! 2. If element jj is significantly smaller than element i
      else if ((val_i - val_j) > tol) then
        indx(kk) = tmp(jj)
        jj = jj + 1

        ! 3. Elements are within tolerance ("equal") -> enforce stability
      else
        if (tmp(ii) <= tmp(jj)) then
          indx(kk) = tmp(ii)
          ii = ii + 1
        else
          indx(kk) = tmp(jj)
      ...
      ! Ensure kk increments on EVERY loop iteration
      kk = kk + 1
