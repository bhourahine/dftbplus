    ...
    if (this%doPerturbation) then
      ...
      if (this%isAtomCoordPerturb) then
        ...
        call this%response%dPsidx(env, this%parallelKS, this%filling, this%eigen, this%eigVecsReal,&
            ...
            & this%pChrgMixerReal, this%taggedWriter, this%tWriteAutotest, autotestTag,&
            & this%tWriteResultsTag, resultsTag, this%tWriteDetailedOut, this%fdDetailedOut%unit,&
            & this%kPoint, this%kWeight, this%iCellVec, this%cellVec, this%tPeriodic,&
            & this%tHelical, this%tMulliken, errStatus)
        if (errStatus%hasError()) then
          call error(errStatus%message)
        end if
      end if
