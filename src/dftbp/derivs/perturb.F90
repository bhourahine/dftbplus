...
contains
  ...
  subroutine dPsidx(this, env, parallelKS, filling, eigvals, eigVecsReal, eigVecsCplx, rhoPrim,&
      ...
      & hybridXc, nNeighbourLC, pChrgMixer, taggedWriter, tWriteAutoTest, autoTestTagFile,&
      & tWriteTaggedOut, taggedResultsFile, tWriteDetailedOut, fdDetailedOut, kPoint, kWeight,&
      & iCellVec, cellVec, tPeriodic, isHelical, tMulliken, errStatus)
      ...
      lpCart: do iCart = 1, 3
        ...
        lpSCC: do while (iSCCiter <= maxSccIter)
          ...
          if (tSccCalc) then
            ...
            call addShift(env, sOmega(:,:,2), over, nNeighbourSK, neighbourList%iNeighbour,&
                & species, orb, iSparseStart, nAtom, img2CentCell, dpotential%intBlock, .true.)
          end if
