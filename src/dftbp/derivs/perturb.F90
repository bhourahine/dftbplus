...
contains
  ...
  !> Response with respect to a external charges at q=0 with respect to atomic postitions
  subroutine wrtExtCharges(this, env, parallelKS, isAutotestWritten, autotestTagFile,&
      & isTagResultsWritten, resultsTagFile, taggedWriter, isBandWritten, fdDetailedOut, filling,&
      & eigvals, eigVecsReal, eigVecsCplx, ham, over, orb, nAtom, species, neighbourList,&
      ...
      if (present(dqdx)) then
        ...
        do iAt = 1, nAtom
          do iCart = 1, 3
            do iS = 1, nSpin
              do jAt = 1, nAtom
                dqdx(jAt, iCart, iAt) = dqdx(jAt, iCart, iAt) + sum(dqOut(:, jAt, iS, iCart, iAt))
              end do
            end do
          end do
        end do
      ...
      if (tWriteDetailedOut .or. isTagResultsWritten) then
        ...
        do iAt = 1, nAtom
          do iCart = 1, 3
            dDipole(:) = 0.0_dp
            do jCart = 1, 3
              dDipole(jCart) = sum(sum(dqOut(:, :nAtom , 1, iCart, iAt), dim=1)&
                  & * coord(jCart, :nAtom))
            end do
