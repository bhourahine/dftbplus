    ...
    lpGeomOpt: do iGeoStep = 0, nGeoSteps
      ...
      lpSCC: do iSccIter = 1, maxSccIter
        ...
        call getDensity(ham, over, neighborList, nNeighbor, iDenseStart, iSparseStart,&
            ...
            & orbitalL, HSqrReal, SSqrReal, iRhoPrim, HSqrCplx, SSqrCplx, iSCCiter, OrbitalShift,&
            & FixLowestEi, tFixLowestEi, tOrbitalShift, rhoSqrReal, storeEigvecsReal,&
            & storeEigvecsCplx)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  ...
  subroutine getDensity(ham, over, neighborList, nNeighbor, iDenseStart, iSparseStart,&
      ...
      & HSqrReal, SSqrReal, iRhoPrim, HSqrCplx, SSqrCplx, iSCCiter, OrbitalShift, FixLowestEi,&
      & tFixLowestEi, tOrbitalShift, rhoSqrReal, storeEigvecsReal, storeEigvecsCplx)
    ...
    real(dp), intent(in) :: OrbitalShift(:), FixLowestEi(:)
    logical, intent(in) :: tFixLowestEi, tOrbitalShift
    ...
    if (tOrbitalShift) then
      if (iSCCiter > 1) then
        if (nSpin == 4) then
          eigen(:,:,1) = eigen(:,:,1) + OrbitalShift(1) * (0.5_dp - filling(:,:,1))
        else
          do iSpin = 1, nSpin
