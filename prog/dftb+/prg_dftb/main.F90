...
#:endif
  ...
  #:if WITH_SCALAPACK
    ! Distribute all eigenvalues to all nodes in current replica via global summation
  ...
  #:if WITH_SCALAPACK
    ...
    call mpifx_allreduceip(env%mpi%interGroupComm, rhoPrim, MPI_SUM)
  #:endif
  ...
  #:if WITH_SCALAPACK
    ...
    call mpifx_allreduceip(env%mpi%interGroupComm, rhoPrim, MPI_SUM)
    if (allocated(iRhoPrim)) then
      call mpifx_allreduceip(env%mpi%interGroupComm, iRhoPrim, MPI_SUM)
    end if
    call mpifx_allreduceip(env%mpi%interGroupComm, energy%atomLS, MPI_SUM)
    if (tMulliken .and. tSpinOrbit .and. .not. tDualSpinOrbit) then
      call mpifx_allreduceip(env%mpi%interGroupComm, orbitalL, MPI_SUM)
    end if
    call env%globalTimer%stopTimer(globalTimers%denseToSparse)
  #:endif
    if (tSpinOrbit .and. .not. tDualSpinOrbit) then
  ...
  subroutine getNextInputCharges(env, pChrgMixer, qOutput, qOutRed, orb, nIneqOrb, iEqOrbitals,&
    ...
    if ((.not. tConverged) .and. (iSCCiter /= maxSccIter .and. .not. tStopScc)) then
      ...
      #:if WITH_MPI
        ...
        call mpifx_allreduceip(env%mpi%interGroupComm, qInpRed, MPI_SUM)
        qInpRed(:) = qInpRed / env%mpi%interGroupComm%size
      #:endif
 ...
 !> Do the linear response excitation calculation.
  ...
  #:if WITH_SCALAPACK
    ...
    call mpifx_allreduceip(env%mpi%interGroupComm, ERhoPrim, MPI_SUM)
  #:endif
