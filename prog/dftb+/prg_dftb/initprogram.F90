  ...
  #:endif
    ...
    if (tSccCalc) then
      ...
      if (nSpin == 1) then
        ...
        call OrbitalEquiv_merge(iEqOrbSCC, iEqOrbSpin, orb, species0, iEqOrbitals)
      ...
      if (tDFTBU) then
        ...
        call OrbitalEquiv_merge(iEqOrbitals, iEqOrbDFTBU, orb, species0, iEqOrbSpin)
    ...
    if (tProjEigenvecs) then
      ...
      do iReg = 1, size(input%ctrl%tShellResInRegion)
        ...
        if (input%ctrl%tOrbResInRegion(iReg) .or. input%ctrl%tShellResInRegion(iReg)) then
          ...
          nOrbRegion = 0
          do ii = 1, nAtomRegion
            nOrbRegion = nOrbRegion + orb%nOrbSpecies(species0(iAtomRegion(ii)))
          end do
          ind = 1
          allocate(tmpir1(nOrbRegion))
    ...
    if (tSccCalc) then
      ...
      if (tReadChrg) then
        if (tDFTBU) then
          if (nSpin == 2) then
            if (tFixEf) then ! do not check charge or magnetisation from file
              call initQFromFile(qInput, fCharges, orb, species0, qBlock=qBlockIn)
            else
              call initQFromFile(qInput, fCharges, orb, species0, nEl = sum(nEl), &
                  & magnetisation=nEl(1)-nEl(2), qBlock=qBlockIn)
            end if
          else
            if (tImHam) then
              if (tFixEf) then
                call initQFromFile(qInput, fCharges, orb, species0, &
                    & qBlock=qBlockIn,qiBlock=qiBlockIn)
              else
                call initQFromFile(qInput, fCharges, orb, species0, nEl = nEl(1), &
                    & qBlock=qBlockIn,qiBlock=qiBlockIn)
              end if
            else
              if (tFixEf) then
        ...
        case (2)
          if (allocated(input%ctrl%initialSpins)) then
            do ii = 1, nAtom
              ...
              ! iterations
              qInput(1:orb%nOrbSpecies(species0(ii)),ii,2) = &
                  & qInput(1:orb%nOrbSpecies(species0(ii)),ii,1) * &
                  & input%ctrl%initialSpins(1,ii) / &
                  & sum(qInput(1:orb%nOrbSpecies(species0(ii)),ii,1))
            end do
          else
            do ii = 1, nAtom
              qInput(1:orb%nOrbSpecies(species0(ii)),ii,2) = &
        ...
        case (4)
          if (tSpin) then
            ...
            do ii = 1, nAtom
              do jj = 1, 3
                qInput(1:orb%nOrbSpecies(species0(ii)),ii,jj+1) = &
                    & qInput(1:orb%nOrbSpecies(species0(ii)),ii,1) * &
      ...
      call OrbitalEquiv_reduce(qInput, iEqOrbitals, orb, species0, qInpRed(1:nIneqOrb))
      if (tDFTBU) then
        call AppendBlock_reduce( qBlockIn,iEqBlockDFTBU, orb, species0, qInpRed )
        if (tImHam) then
          call AppendBlock_reduce( qiBlockIn,iEqBlockDFTBULS, orb, species0, qInpRed, skew=.true. )
        end if
      end if
