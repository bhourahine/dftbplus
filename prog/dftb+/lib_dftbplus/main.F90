  ...
  #:endif
    ...
    if (tWriteResultsTag) then
      call writeResultsTag(resultsTag, dftbEnergy(deltaDftb%iFinal), derivs, chrgForces, nEl, Ef,&
          ...
          & tMulliken, qOutput, q0, taggedWriter, cm5Cont, deltaDftb, dipoleMoment)
    end if
        ...
        #:endif
          ...
          if (allocated(onSiteElements) .and. (iSCCIter > 1 .or. tReadChrg)) then
            call addOnsShift(potential%intBlock, potential%iOrbitalBlock, qBlockIn, qiBlockIn,&
                & onSiteElements, species, orb, q0=q0)
          end if
...
#:endif
  ...
  subroutine getHamiltonianLandEnergyL(env, denseDesc, sccCalc, orb, species, neighbourList, &
    ...
    do iL = 1, reks%Lmax
      ...
      call sccCalc%updateCharges(env, reks%qOutputL(:,:,:,iL), orb, species, q0=q0)
