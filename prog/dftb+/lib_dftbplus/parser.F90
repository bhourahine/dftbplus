  ...
  #:endif
    ...
    if (ctrl%tSCC) then
     !call getChildValue(node, "Multipole", ctrl%isMultiPole, .false.)
      call getChildValue(node, "Multipole", value1, "", child=child)
      if (associated(value1)) then
        call getNodeName(value1, buffer)
        select case(char(buffer))
        case("onecenterapproximation")
          ...
          if (associated(child2)) then
            do iSp1 = 1, geo%nSpecies
              call getChildValue(child2, trim(geo%speciesNames(iSp1)), ctrl%atomicDIntgrlScaling(iSp1), 1.0_dp)
            end do
          end if
          ...
          if (associated(child2)) then
            do iSp1 = 1, geo%nSpecies
              call getChildValue(child2, trim(geo%speciesNames(iSp1)), ctrl%atomicQIntgrlScaling(iSp1), 1.0_dp)
            end do
          end if
