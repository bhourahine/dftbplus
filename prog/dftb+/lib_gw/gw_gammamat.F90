...
module dftbp_GammaMat
  use dftbp_accuracy
  use dftbp_shortgammafuncs, only : expGamma
...
contains
  ...
  subroutine gammaM_real(env, nAtom, mAngAtom, specie, coord, hubbU, gamma)
    ...
    !> Electron repulsion constants = Hubbard values
      ...
      do iL1 = 1, mAngAtom(iAt1)+1
        ...
        do iAt2 = iAt1, nAtom
          ...
          do iL2 = 1, mAngAtom(iAt2)+1
            ...
            gamma(indL(iAt2)+iL2,indL(iAt1)+iL1) = invRMat(iAt2,iAt1) - expGamma(r12,U2,U1)
          enddo
        enddo
      enddo
    enddo
