...
contains
  ...
  subroutine dPsidx(env, parallelKS, filling, eigvals, eigVecsReal, eigVecsCplx, rhoPrim,&
      ...
      & iCellVec, cellVec, tPeriodic, tMulliken, dqGross, dqNet)
    ...
    !> Gross (Mulliken) atomic charge derivatives
    real(dp), intent(inout), allocatable :: dqGross(:,:,:,:,:)

    !> Net (onsite) atomic charge derivatives
    real(dp), intent(inout), allocatable :: dqNet(:,:,:,:)
  ...
  #:endif
    ...
    if (allocated(dqNet)) then
      @:ASSERT(all(shape(dqNet) == [nAtom, size(ham, dim=2), 3, nAtom] ))
    endif
    if (allocated(dqGross)) then
      @:ASSERT(all(shape(dqGross) == [orb%mOrb, nAtom, size(ham, dim=2), 3, nAtom] ))
    endif
    if (allocated(dqNet) .or. allocated(dqGross)) then
      @:ASSERT(tSccCalc .or. tMulliken)
    end if
  ...
  #:endif
    ...
    lpAtom: do iAt = 1, nAtom
      ...
      lpCart: do iCart = 1, 3
        ...
        if (allocated(dqNet)) then
          dqNet(:, :, iCart, iAt) = 0.0_dp
          do iS = 1, nSpin
            call getOnsitePopulation(drho(:,iS), orb, iSparseStart, dqNet(:, iS, iCart, iAt))
          end do
        end if
  ...
  #:endif
    ...
    if (tMulliken .or. tSccCalc) then
      ...
      if (allocated(dqNet)) then
        write(stdOut, *)'Net charge derivatives'
        do iAt = 1, nAtom
          write(stdOut,"(A,I0)")'/d Atom_',iAt
          do iS = 1, nSpin
            do jAt = 1, nAtom
              write(stdOut, *)jAt, -dqNet(jAt, iS, :, iAt)
            end do
            write(stdOut, *)
          end do
        end do
